...
contract HarvestFinanceStrat is IStrat {
    ...
    function invest() external override onlyVault {
        ...
        if(balance > buffer) {
            uint max = fToken.availableDepositLimit();
    ...
    function sharesForAmount(uint amount) internal view returns (uint) {
        return amount.mul(fToken.totalSupply()).div(fToken.totalAssets());
    }
